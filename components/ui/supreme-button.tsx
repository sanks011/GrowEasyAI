...
interface SupremeButtonProps {
  ...
  onClick?: () => void
  variant?: "primary" | "secondary" | "outline" | "ghost" | "destructive" | "premium" | "executive"
  size?: "sm" | "md" | "lg" | "xl"
  className?: string
  disabled?: boolean
  loading?: boolean
...
export function SupremeButton({
  ...
  return (
    <motion.button
      ...
      whileHover={!disabled ? { scale: 1.02, y: elevated ? -3 : 0 } : {}}
    ...
    >
      ...
      {glow && (
        <motion.div
          className="absolute -inset-1 bg-gradient-to-r from-blue-600/30 via-purple-600/30 to-cyan-600/30 rounded-xl blur-sm opacity-0 group-hover:opacity-100"
          animate={{
            background: [
      ...
      {/* Particle effects for executive variant */}
      {variant === "executive" && (
